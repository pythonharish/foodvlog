...
def cart_details(request,tot=0,count=0,cart_itmes=None):
    try:
        ...
        ct_itmes=items.objects.filter(cart=ct,active=True)
        for i in ct_itmes:
            tot +=(i.prodt.price*i.quantity)
            count +=i.quantity
    except object.DoesNotExist:
        pass
    return render(request,'cart.html',{'ci':ct_itmes,'t':tot,'cn':count})
def c_id(request):
...
def add_cart(request,product_id):
    ...
    try:
        ...
        c_items=items.objects.create(prodt=prod,cart=ct,quantity=1)
        c_items.save()
